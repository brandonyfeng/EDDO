...
from dl_utils import pixel_coords, crop_to, arcsec2rad, partial_MFT,calc_snr, circular_mask
...
def run_optimization(data_dir, root_dir, scene_name,exp_name, star_offset_x, star_offset_y, use_ptt,reference_file, measurement_file,lr,iters, vis_freq,ref_cutoff_iter, num_wl, oversample,num_det_px,sci_targ_name,num_ints,ref_which_int, sci_which_int,use_linear_interp_opd,no_median,measurement_noisemap,reference_noisemap,use_simulated_data,px_mask_file,fit_Lyot,fit_everything,stage_fluxpos_cutoff_iter,smooth,OPD_loss_weight,freeze_position,be_normal,sci_lr_weight, injected_companion=None, pre_initialized_state=None):
    ...
    if use_ptt:
        ...
        prop_models = [PointPropagate(aperture, lyot, fpm, nircam_OPD, prop_args,oversample=oversample, use_ptt=entrance_OPD_PTT, num_det_px=num_det_px).to(DEVICE) for _ in range(num_ints)]
    elif use_linear_interp_opd:
        entrance_OPD_LinearInterp = LinearInterpOPD(wfe_batch_list[0], wfe_batch_list[1]).to(DEVICE)
        prop_models = [PointPropagate(aperture, lyot, fpm, nircam_OPD, prop_args,oversample=oversample, use_ptt=entrance_OPD_LinearInterp, num_det_px=num_det_px).to(DEVICE) for _ in range(num_ints)]
    else:
        prop_models = [PointPropagate(aperture, lyot, fpm, nircam_OPD, prop_args,oversample=oversample, num_det_px=num_det_px).to(DEVICE) for _ in range(num_ints)]
    ...
    real_im = np.load(f'{data_dir}/real_data/{measurement_file}')[sci_which_int, (edge1-shift_y):(edge2-shift_y), (edge1-shift_x):(edge2-shift_x)]
    ...
    for i in tbar:
        ...
        #     prior_loss+=gaussian_log_prior(p.angle_offsets(), position_center_prior, sigma_center_prior)
        
        # if i%100==0:
        #     with torch.no_grad():
        #         print('loss is in this step: ', loss)
        #         print('prior loss from position is: ', prior_loss)
        ...
        # if i%100==0:
            # print('HERE')
            # with torch.no_grad():
            #     for name, j in prop_models[0].charge_diffusion.named_parameters():
        ...
        #         for t in p_model.angle_offsets.parameters():
        #             if t.requires_grad:
        #                 t.grad *= 0.000000000001
        # if i%100==0:
        #     for p_model in prop_models:
        #             print('FLUX FACTOR CORRECTION IS : ',p_model.flux_correction.data)
        #     print('---HERE!! BEFORE')
        #     with torch.no_grad():
        #         print(f"Flux: {prop_models[0].charge_diffusion.log_sigma.item()} | Grad: {prop_models[0].charge_diffusion.log_sigma.grad.item()} | Req grad??{prop_models[0].charge_diffusion.log_sigma.requires_grad} ")
        
        if i%100==0:
            with torch.no_grad():
                for p_model in prop_models:
                        print('ANGLES OFFSET BEFORE IS: ',p_model.angle_offsets())
        optimizer.step()

        if i%10==0:
            with torch.no_grad():
                for p_model in prop_models:
                        print('ANGLES OFFSET AFTER IS: ',p_model.angle_offsets(), ' with grad = ', p_model.angle_offsets.data.grad)
    ...
    if fit_Lyot:
        lyot_arr = torch.stack(lyot_arr)[::5]
    ...
    #                 'FFMPEG', **{'macro_block_size': None, 'fps': 30, })
    if pre_initialized_state is None:
        # Return pre_initialization too
        print('FINAL REF STATE IS YEAH ', final_ref_state)
        if ref_cutoff_iter > iters:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_radii', default=None, type=int)
    parser.add_argument('--num_angles', default=1, type=int)
    parser.add_argument('--min_radii_px', default=1., type=float)
    ...
    parser.add_argument('--mask_HIP_inSNR', action='store_true')
    parser.add_argument('--blur_annulus_SNR', action='store_true')
    parser.add_argument('--blur_before_signal', action='store_true')
    parser.add_argument('--use_both_sci_ints', action='store_true')
    

    args = parser.parse_args()

    pixel_scale = 0.0625953036
    vis_dir = f'{args.root_dir}/vis/{args.scene_name}/{args.exp_name}'
    vis_dir_iterations = f'{args.root_dir}/vis/{args.scene_name}/{args.exp_name}/ITERATIONS'
    os.makedirs(vis_dir_iterations, exist_ok=True)
    DEVICE = 'cuda'
    if args.px_mask_file is not None:
        px_mask = np.load(f'{args.data_dir}/real_data/{args.px_mask_file}')
        if args.num_det_px != 80:
            edge1, edge2 = 40 - int(args.num_det_px/2), 40 + int(args.num_det_px/2)
            px_mask = px_mask[edge1:edge2, edge1:edge2]
        # px_mask = torch.from_numpy(px_mask.astype(np.float32)).to(DEVICE)[None]
        px_mask_nanned = np.where(px_mask == 0., np.nan, px_mask)
    # Number of pixels in the wavefront
    residual, _, pre_initialized_conditions = run_optimization(args.data_dir, args.root_dir, args.scene_name,args.exp_name, args.star_offset_x, args.star_offset_y, args.use_ptt,args.reference_file, args.measurement_file,args.lr,args.iters, args.vis_freq,args.ref_cutoff_iter, args.num_wl, args.oversample,args.num_det_px,args.sci_targ_name,args.num_ints,args.ref_which_int, args.sci_which_int,args.use_linear_interp_opd,args.no_median,args.measurement_noisemap,args.reference_noisemap,args.use_simulated_data,args.px_mask_file,args.fit_Lyot,args.fit_everything,args.stage_fluxpos_cutoff_iter,args.smooth,args.OPD_loss_weight,args.freeze_position,args.be_normal,args.sci_lr_weight, injected_companion=None)
    if args.use_both_sci_ints:
        # Repeat on the second science integration
        residual2, _= run_optimization(args.data_dir, args.root_dir, args.scene_name,args.exp_name, args.star_offset_x, args.star_offset_y, args.use_ptt,args.reference_file, args.measurement_file,args.lr,args.iters, args.vis_freq,args.ref_cutoff_iter, args.num_wl, args.oversample,args.num_det_px,args.sci_targ_name,args.num_ints,args.ref_which_int, args.sci_which_int + 1,args.use_linear_interp_opd,args.no_median,args.measurement_noisemap,args.reference_noisemap,args.use_simulated_data,args.px_mask_file,args.fit_Lyot,args.fit_everything,args.stage_fluxpos_cutoff_iter,args.smooth,args.OPD_loss_weight,args.freeze_position,args.be_normal,args.sci_lr_weight, injected_companion=None, pre_initialized_state=pre_initialized_conditions)
        residual = residual/2 + residual2/2

    if args.num_radii is not None:
        torch.cuda.empty_cache()

        if args.mask_HIP_inSNR:
            x_pos, y_pos = -7.2 * pixel_scale, 11 * pixel_scale # hand tuned for HIP 65426 in the pre-rotated frame
            # print('-------circular mask')
            masked_residual = circular_mask(residual, x_pos, y_pos, 1.0, np.nan)
        else:
        ...
        plt.subplot(122)
        plt.imshow(masked_residual, origin='lower')

        plt.tight_layout()
        plt.savefig(f'{vis_dir}/masked_result.png')
        plt.close()
        # print('-------figure')

        # Compute contrast curve

        # CONTRAST = 1 FOR HIP65426 is 9695104.369333692 MJy/sr
        
        mask335r_profile = np.genfromtxt(f'{args.data_dir}/MASK335R.csv', delimiter=',')
        if args.px_mask_file is not None:
            seps, contrasts_initial = meas_contrast(masked_residual*px_mask_nanned, 0, int(args.num_det_px/2), int(int(args.num_det_px/2)/args.num_radii), low_pass_filter=0.3)
        else:
            seps, contrasts_initial = meas_contrast(masked_residual, 0, int(args.num_det_px/2), int(int(args.num_det_px/2)/args.num_radii), low_pass_filter=0.3)
        contrast_new_test_weighing = np.interp(seps * pixel_scale,mask335r_profile[:,0], mask335r_profile[:,1],left=0., right=1.)
        contrasts_mask_cal_initial = contrasts_initial / contrast_new_test_weighing
        seps_arcsec = seps * pixel_scale
        np.save(f'{vis_dir}/UNCAL_flux_throughoutcorrected_contrastcurve_flux.npy', contrasts_mask_cal_initial)
        plt.figure()
        plt.plot(seps_arcsec, contrasts_mask_cal_initial)
        plt.xlabel('Separation [arcsec]')
        plt.ylabel('5 sigma curve [MJy/sr]')
        plt.yscale('log')
        plt.grid()
        plt.savefig(f'{vis_dir}/detection_limit.png')
        plt.close()
        ...
        signal_loss = []
        mean_snrs_achieved = []
        for i in range(args.num_radii):
            curr_rad_px = args.min_radii_px + (args.num_det_px/2 - args.min_radii_px) / args.num_radii * i # closest one at 2 pixels, yeah
            # curr_rad_px = 30. # TODO REMOVE LATER
            print('CURR RAD PX IS ', curr_rad_px)
            radii_for_means.append(curr_rad_px)
            snrs = []
            fluxes_curr_radii_injected = []
            fluxes_curr_radii_recovered = []
            signal_losses_curr_radii = []
            for j in range(args.num_angles):
                pos_x_px, pos_y_px = curr_rad_px * np.sin(2. * np.pi / args.num_angles * j + np.pi),-curr_rad_px * np.cos(2. * np.pi / args.num_angles * j + np.pi)
                print('seps are ', seps)
                curr_flux = np.interp(np.abs(curr_rad_px) * pixel_scale,seps * pixel_scale, contrasts_mask_cal_initial,left=None, right=None)
                # contrasts_mask_cal_initial = contrasts_initial / contrast_new_test_weighing
                curr_flux*=2. # TODO REMOVE LATER
                # curr_flux*=50. # TODO REMOVE LATER
                radii_list_px.append(curr_rad_px)
                angle_list_deg.append(2. * np.pi / args.num_angles * j + np.pi)
                
                # fluxes_curr_radii_injected.append(curr_flux)
                planet_injection = {'pos_x_px': pos_x_px, 'pos_y_px': -pos_y_px, 'flux': curr_flux, 'radius':i,'theta':j, 'iter_num':0}
                residual, injected_peak_planet_flux = run_optimization(args.data_dir, args.root_dir, args.scene_name,args.exp_name, args.star_offset_x, args.star_offset_y, args.use_ptt,args.reference_file, args.measurement_file,args.lr,args.iters, args.vis_freq,args.ref_cutoff_iter, args.num_wl, args.oversample,args.num_det_px,args.sci_targ_name,args.num_ints,args.ref_which_int, args.sci_which_int,args.use_linear_interp_opd,args.no_median,args.measurement_noisemap,args.reference_noisemap,args.use_simulated_data,args.px_mask_file,args.fit_Lyot,args.fit_everything,args.stage_fluxpos_cutoff_iter,args.smooth,args.OPD_loss_weight,args.freeze_position,args.be_normal,args.sci_lr_weight, injected_companion=planet_injection, pre_initialized_state=pre_initialized_conditions)
                # flux_list_injected.append(injected_peak_planet_flux)
                fluxes_curr_radii_injected.append(injected_peak_planet_flux)
                if args.use_both_sci_ints:
                    # Repeat on the second science integration
                    residual2, _ = run_optimization(args.data_dir, args.root_dir, args.scene_name,args.exp_name, args.star_offset_x, args.star_offset_y, args.use_ptt,args.reference_file, args.measurement_file,args.lr,args.iters, args.vis_freq,args.ref_cutoff_iter, args.num_wl, args.oversample,args.num_det_px,args.sci_targ_name,args.num_ints,args.ref_which_int, args.sci_which_int+1,args.use_linear_interp_opd,args.no_median,args.measurement_noisemap,args.reference_noisemap,args.use_simulated_data,args.px_mask_file,args.fit_Lyot,args.fit_everything,args.stage_fluxpos_cutoff_iter,args.smooth,args.OPD_loss_weight,args.freeze_position,args.be_normal,args.sci_lr_weight, injected_companion=planet_injection, pre_initialized_state=pre_initialized_conditions)
                    residual = residual/2 + residual2/2
                ...
                # Mask real companion
                if args.mask_HIP_inSNR:
                    x_pos_real, y_pos_real = -7.2 * pixel_scale, 11 * pixel_scale # hand tuned for HIP 65426 in the pre-rotated frame
                    # print('-------circular mask')
                    masked_residual = circular_mask(residual, x_pos_real, y_pos_real, 1.0, np.nan)
                else:
                    masked_residual = residual 
                if args.px_mask_file is not None:
                    curr_snr, annulus, signal_blob = calc_snr(masked_residual*px_mask_nanned, pos_x_px*pixel_scale, -pos_y_px*pixel_scale,0.3,curr_rad_px*pixel_scale, blur_annulus=args.blur_annulus_SNR, blur_before_signal=args.blur_before_signal)
                else:
                    curr_snr, annulus, signal_blob = calc_snr(masked_residual, pos_x_px*pixel_scale, -pos_y_px*pixel_scale,0.3,curr_rad_px*pixel_scale, blur_annulus=args.blur_annulus_SNR, blur_before_signal=args.blur_before_signal)
                
                fluxes_curr_radii_recovered.append(np.nanmax(signal_blob))
                plt.figure(figsize=[15,10])
                plt.subplot(231)
                plt.imshow(residual, origin='lower')
                plt.title(f'Sum, peak = {np.nansum(residual):.2f},{np.nanmax(residual):.2f} (orig)')
                plt.subplot(232)
                plt.imshow(masked_residual, origin='lower', vmin=-10)
                ...
                plt.subplot(235)
                plt.imshow(signal_blob, origin='lower')
                plt.title(f'Sum, peak = {np.nansum(signal_blob):.2f},{np.nanmax(signal_blob):.2f},\n pos x, pos y, flux ={pos_x_px:.2f},{pos_y_px:.2f},{curr_flux:.2f} (signal injected planet)')
                whole_annulus = np.where(~np.isnan(signal_blob), signal_blob, annulus)
                plt.subplot(236)
                plt.imshow(whole_annulus, origin='lower')
                plt.title(f'Whole annulus. Peak = {np.nanmax(whole_annulus):.2f}')
                
                plt.tight_layout()
                plt.savefig(f'{vis_dir_iterations}/vis_measurement_INJECTED_SNR_r{i}_theta{j}_flux{0}.png')
                plt.close()
                snrs.append(curr_snr)

            mean_snr_currad = np.array(snrs).mean()
            mean_fluxes_curr_radii_injected = np.array(fluxes_curr_radii_injected).mean()
            mean_fluxes_curr_radii_recovered = np.array(fluxes_curr_radii_recovered).mean()
            signal_losses_curr_radii = mean_fluxes_curr_radii_recovered / mean_fluxes_curr_radii_injected

            # Put into data
            mean_snrs_achieved.append(mean_snr_currad)
            flux_list_injected.append(mean_fluxes_curr_radii_injected)
            flux_list_recovered.append(mean_fluxes_curr_radii_recovered)
            signal_loss.append(signal_losses_curr_radii)
            # if mean_snr_currad < 5. or mean_snr_currad > 6.:
            #     too_faint_or_bright = True
            #     counter = 0
            #     # curr_flux = np.interp(np.abs(curr_rad_px) * pixel_scale,seps * pixel_scale, contrasts_mask_cal_initial,left=0., right=1.)
            #     # curr_flux*=10000.
            #     while too_faint_or_bright:
            #         counter+=1
            #         print('-----ITERATION = ', counter, f' FOR RADIUS i = {i} out of {args.num_radii}', 'SNR = ',mean_snr_currad)
            #         snrs = []
            #         # Weigh flux to get a marginal detection:
            #         # If it is too faint, bump up the brightness to get an actual detection;
            #         # If it is too bright, dial down the brightness to get a detection closer to the limit
            #         curr_flux *= 5.5 / mean_snr_currad
            #         for j in range(args.num_angles):
            #             pos_x_px, pos_y_px = curr_rad_px * np.sin(2. * np.pi / args.num_angles * j + np.pi),-curr_rad_px * np.cos(2. * np.pi / args.num_angles * j + np.pi)
            #             # contrasts_mask_cal_initial = contrasts_initial / contrast_new_test_weighing
            #             print('seps are ', seps)
            #             planet_injection = {'pos_x_px': pos_x_px, 'pos_y_px': -pos_y_px, 'flux': curr_flux, 'radius':i,'theta':j, 'iter_num':counter}
            #             residual = run_optimization(args.data_dir, args.root_dir, args.scene_name,args.exp_name, args.star_offset_x, args.star_offset_y, args.use_ptt,args.reference_file, args.measurement_file,args.lr,args.iters, args.vis_freq,args.ref_cutoff_iter, args.num_wl, args.oversample,args.num_det_px,args.sci_targ_name,args.num_ints,args.ref_which_int, args.sci_which_int,args.use_linear_interp_opd,args.no_median,args.measurement_noisemap,args.reference_noisemap,args.use_simulated_data,args.px_mask_file,args.fit_Lyot,args.fit_everything,args.stage_fluxpos_cutoff_iter,args.smooth,args.OPD_loss_weight, args.freeze_position,args.be_normal,args.sci_lr_weight, injected_companion=planet_injection)
            #             if args.use_both_sci_ints:
            #                 # Repeat on the second science integration
            #                 residual2 = run_optimization(args.data_dir, args.root_dir, args.scene_name,args.exp_name, args.star_offset_x, args.star_offset_y, args.use_ptt,args.reference_file, args.measurement_file,args.lr,args.iters, args.vis_freq,args.ref_cutoff_iter, args.num_wl, args.oversample,args.num_det_px,args.sci_targ_name,args.num_ints,args.ref_which_int, args.sci_which_int+1,args.use_linear_interp_opd,args.no_median,args.measurement_noisemap,args.reference_noisemap,args.use_simulated_data,args.px_mask_file,args.fit_Lyot,args.fit_everything,args.stage_fluxpos_cutoff_iter,args.smooth,args.OPD_loss_weight, args.freeze_position,args.be_normal,args.sci_lr_weight, injected_companion=planet_injection)
            #                 residual = residual/2 + residual2/2
            #             torch.cuda.empty_cache()
            #             # Mask real companion
            #             if args.mask_HIP_inSNR:
            #                 x_pos_real, y_pos_real = -6.8 * pixel_scale, 11 * pixel_scale # hand tuned for HIP 65426 in the pre-rotated frame
            #                 # print('-------circular mask')
            #                 masked_residual =  circular_mask(residual, x_pos_real, y_pos_real, 1.0, np.nan)
            #             else:
            #                 masked_residual =  residual
            ...
            #             plt.title(f'Sum, peak = {np.nansum(signal_blob):.2f},{np.nanmax(signal_blob):.2f},\n pos x, pos y, flux ={pos_x_px:.2f},{pos_y_px:.2f}.{curr_flux:.2f} (signal injected planet)')
            #             plt.tight_layout()
            #             plt.savefig(f'{vis_dir_iterations}/vis_measurement_INJECTED_SNR_r{i}_theta{j}_flux{counter}.png')
            #             plt.close()
            #             snrs.append(curr_snr)
                    
            #         mean_snr_currad = np.array(snrs).mean()

            #         if mean_snr_currad > 5. and mean_snr_currad < 6.:
            #             too_faint_or_bright = False
            #         elif counter == 12:
            #             too_faint_or_bright = False # GIVE UP AFTER 10 ATTEMPTS!

            # flux_list.append(curr_flux)
            

        radii_list_px = np.array(radii_list_px)
        # flux_list = np.array(flux_list)
        mean_snrs_achieved = np.array(mean_snrs_achieved)
        flux_list_injected = np.array(flux_list_injected)
        flux_list_recovered = np.array(flux_list_recovered)
        signal_loss = np.array(signal_loss)

        algo_throughput_correction = np.interp(seps,radii_list_px, signal_loss,left=np.nanmean(signal_loss[:4]), right=None) # see what happens with default values
        contrasts_mask_cal_initial_throughputcorrected = contrasts_mask_cal_initial / algo_throughput_correction

        plt.figure()
        plt.plot(seps * pixel_scale, contrasts_mask_cal_initial_throughputcorrected)
        plt.xlabel('Separation [arcsec]')
        plt.ylabel('5 sigma curve [MJy/sr]')
        plt.yscale('log')
        plt.grid()
        plt.savefig(f'{vis_dir}/detection_limit_CALIBRATED_AND_THROUGHPUTCORRECTED.png')
        plt.close()

        np.save(f'{vis_dir}/cal_flux_lim_radii_px.npy', radii_list_px)
        # np.save(f'{vis_dir}/cal_flux_lim_flux.npy', flux_list)
        np.save(f'{vis_dir}/cal_flux_lim_mean_snr.npy', mean_snrs_achieved)
        np.save(f'{vis_dir}/cal_flux_flux_list_injected.npy', flux_list_injected)
        np.save(f'{vis_dir}/cal_flux_flux_list_recovered.npy', flux_list_recovered)
        np.save(f'{vis_dir}/cal_flux_signal_loss.npy', signal_loss)

        np.save(f'{vis_dir}/cal_flux_throughoutcorrected_contrastcurve_seps.npy', seps * pixel_scale)
        np.save(f'{vis_dir}/cal_flux_throughoutcorrected_contrastcurve_flux.npy', contrasts_mask_cal_initial_throughputcorrected)


                    


        # Now, run the optimizations again for every injected planet, until the average SNR per radius is above 5



        # Mask planet in image (write code to get a circular region given coordinates)
        # Compute 5 sigma contrast at each annulus, as indicated by the num_radii. Weigh by transmission of fpm
        # Get desired fluxes: as given by the contrast curves, maybe a 6-8 sigma contrast value?
        # Repeat above, but injecting planets in the desired locations with the desired fluxes
        # check resulting SNR and brightness of each run, using also the circular region given coordinates code
        #
