...
class PointPropagate(nn.Module):
    def __init__(self, aperture, lyot, fpm, nircam_opd, args, use_ptt = None):
        ...
        (npixels, wavelengths, true_pixel_scale, psf_npix, psf_pixel_scale, focal_length, shift, pixel, inverse, der_loss) = args
    ...
    def forward(self, wavefront_list, wfe, wl_weights, wavelenghts):
        ...
        if self.der_loss:
            ...
            return output, nu_output, alpha_output, beta_output, al_uq, ep_uq
        else:
            return output
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', default='exp_hip', type=str)
    ...
    parser.add_argument('--star_offset_x', default=1.5, help='Initial star offset (in pixel)', type=float)
    parser.add_argument('--star_offset_y', default=-3.0, help='Initial star offset (in pixel)', type=float)
    parser.add_argument('--use_ptt', action='store_true')
    parser.add_argument('--der_loss', default=True)
    ...
    prop_args = (npixels, wlen_weights[0], true_pixel_scale, psf_npix, psf_pixel_scale, focal_length, shift, pixel, inverse, args.der_loss)
    ...
    print(f'est_residual shape: {est_residual.shape}')
    ...
    for p_model in prop_models:
        ...
        if p_model.der_loss:
            optics_params += [p_model.alpha, p_model.beta, p_model.nu]
    ...
    for i in tbar:
        ...
        if args.der_loss:
            results = [p_model(wavefronts_list1, wfe_batch, wlen_weights[1], wlen_weights[0]) for p_model in prop_models]
            pred_1, v_1, alpha_1, beta_1, al_uq_1, ep_uq_1 = zip(*results)
        ...
        if i % args.vis_freq == 0 and i > 0:
            if args.der_loss:
                np.save(f'{vis_dir}/ep_arr_{i}.npy', ep_uq_1.squeeze(0).detach().cpu().numpy())
                np.save(f'{vis_dir}/al_arr_{i}.npy', al_uq_1.squeeze(0).detach().cpu().numpy())
            plt.imsave(f'{vis_dir}/vis_est_res_{i}.png', progress_arr[-1].cpu().numpy(), cmap='viridis', origin='lower')
            np.save(f'{vis_dir}/est_res_{i}.npy', progress_arr[-1].cpu().numpy())
