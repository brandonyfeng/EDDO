...
if __name__ == "__main__":
    ...
    real_im = np.load(f'{args.data_dir}/real_data/{args.reference_file}')[args.ref_which_int, (edge1-shift_y):(edge2-shift_y), (edge1-shift_x):(edge2-shift_x)]
    ...
    if args.track_injected_planet: 
        ...
        injected_companion = {'pos_x_px': 0., 'pos_y_px': -17., 'flux': 3.}#, 'radius':i,'theta':j, 'iter_num':0}
        with torch.no_grad():
        ...
        print('planet np max is ', np.nanmax(pred_planet_np))
        print('peak flux star is ', peak_flux_star.detach().numpy())
        print('peak planet flux input is ',injected_companion['flux'])
        print('scaling factor to pred_planet_np is ', injected_companion['flux'] /0.0037 / torch.clone(peak_flux_star).detach().numpy())
        ...
        pred_planet_scaled = pred_planet_scaled[None]
        original_injected_peak = np.nanmax(pred_planet_scaled)
        print('resulting scaled peak planet is ', original_injected_peak)
        ...
        # plt.subplot(132)
        plt.imshow(np.squeeze(pred_planet_scaled), origin='lower')
        ...
        plt.tight_layout()
        plt.savefig(f'{vis_dir}/vis_INJECTEDPLANET.png')
        plt.close()
    ...
    real_im = np.load(f'{args.data_dir}/real_data/{args.measurement_file}')[args.sci_which_int, (edge1-shift_y):(edge2-shift_y), (edge1-shift_x):(edge2-shift_x)]
    real_im = real_im.astype(np.float32)
    ...
    if args.track_injected_planet: 
        real_im += pred_planet_scaled.copy()
        sci_injected_peaks = []
        sci_iters = []
        sci_signal_loss = []
        sci_snr = []
    ...
    if args.measurement_noisemap is not None:
        real_im_noisemap = np.load(f'{args.data_dir}/real_data/{args.measurement_noisemap}')[args.sci_which_int, (edge1-shift_y):(edge2-shift_y), (edge1-shift_x):(edge2-shift_x)]
        ...
        if i%50 == 0:
            if args.track_injected_planet: 
                ...
                if i > cutoff_iter:
                    ...
                    residual = np.squeeze(residual)
                    masked_residual = residual
                # masked_residual = masked_residual.detach().cpu().numpy()
                # masked_residual = np.squeeze(masked_residual)

                print('masked residual shape is ', masked_residual.shape)
                if args.px_mask_file is not None:
                    px_mask_nanned = np.where(px_mask_numpy == 0., np.nan, px_mask_numpy)
                    curr_snr, annulus, signal_blob = calc_snr(masked_residual*px_mask_nanned, injected_companion['pos_x_px']*psf_pixel_scale_arcsec, injected_companion['pos_y_px']*psf_pixel_scale_arcsec,0.3,np.sqrt(injected_companion['pos_x_px']**2 + injected_companion['pos_y_px']**2)*psf_pixel_scale_arcsec, blur_annulus=args.blur_annulus_SNR, blur_before_signal=args.blur_before_signal)
                else:
                    curr_snr, annulus, signal_blob = calc_snr(masked_residual, injected_companion['pos_x_px']*psf_pixel_scale_arcsec, injected_companion['pos_y_px']*psf_pixel_scale_arcsec,0.3,np.sqrt(injected_companion['pos_x_px']**2 + injected_companion['pos_y_px']**2)*psf_pixel_scale_arcsec, blur_annulus=args.blur_annulus_SNR, blur_before_signal=args.blur_before_signal)
                ...
                plt.tight_layout()
                plt.savefig(f'{vis_dir_iterations_current}/vis_measurement_INJECTED_iter{i}_{label}.png')
                plt.close()

                if i > cutoff_iter:
                    sci_injected_peaks.append(np.nanmax(signal_blob))
                    sci_signal_loss.append(np.nanmax(signal_blob)/original_injected_peak)
                    sci_snr.append(curr_snr)
                    sci_iters.append(i)
                else:
                    ref_injected_peaks.append(np.nanmax(signal_blob))
                    ref_signal_loss.append(np.nanmax(signal_blob)/original_injected_peak)
