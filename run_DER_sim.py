...
class PointPropagate(nn.Module):
    def __init__(self, aperture, lyot, fpm, nircam_opd, der_loss, *args):
        super().__init__()
        ...
        psf_npix = args[3]  # Number of pixels in the PSF

        if self.der_loss:
            self.alpha = nn.Parameter(torch.ones((psf_npix, psf_npix), dtype=torch.float64), requires_grad=True)
            self.beta = nn.Parameter(torch.ones((psf_npix, psf_npix), dtype=torch.float64), requires_grad=True)
            self.nu = nn.Parameter(torch.ones((psf_npix, psf_npix), dtype=torch.float64), requires_grad=True)
    ...
    def forward(self, wavefront, wfe):
        ...
        if self.der_loss:
            nu_output =  F.softplus(self.nu) + 1
            alpha_output = F.softplus(self.alpha) + 1
            beta_output = F.softplus(self.beta) 
            al_uq = beta_output / (alpha_output - 1)
            ep_uq = beta_output / ((alpha_output - 1) * nu_output)
            return out, nu_output, alpha_output, beta_output, al_uq, ep_uq
        else:
            return out
...
if __name__ == "__main__":
    ...
    parser.add_argument('--der_loss', default=True, type=bool)
    ...
    p1_model = PointPropagate(aperture, lyot, fpm, nircam_OPD, der_loss, *prop_args)
    p2_model = PointPropagate(aperture, lyot, fpm, nircam_OPD, der_loss, *prop_args)
    ...
    out_1, out_2 = [], []
    if der_loss:
        out_1, _, _, _, al_uq_1, ep_uq_1 = p1_model(plane_wave_1, sampledWFEs)
        out_2, _, _, _, al_uq_2, ep_uq_2 = p2_model(plane_wave_2, sampledWFEs)
    else:
    ...
    if args.shot_noise > 0:
        ...
        np.save(f'{vis_dir}/noise.npy', noise_np)
        np.save(f'{vis_dir}/shot_noise.npy', args.shot_noise * torch.sqrt(out).detach().cpu().numpy())
    ...
    with torch.no_grad():
        ...
        for batch in range(0, N, 32):
            ...
            if der_loss:
                pred_1, _, _, _, _, _ = p1_model(plane_wave_1, cur_wfe)
            else:
                pred_1 = p1_model(plane_wave_1, cur_wfe)
            e_pred.append(pred_1.detach().cpu())
        e_pred = torch.cat(e_pred)
    
    print(f'Device out {out.device}. Device e_pred {e_pred.device}')
    out.to(DEVICE)
    e_pred = e_pred.to(DEVICE)
    ...
    wfe_err, img_err = F.l1_loss(wfe_batch, wfe_gt), F.l1_loss(e_pred, observations)
    img_loss_arr, wfe_err_arr = [], []
    img_loss_arr.append(img_err.item())
    wfe_err_arr.append(wfe_err.item())

    # Set up the optimizer and scheduler
    opt_params = [wfe_batch]
    if der_loss:
        #opt_params += [p1_model.nu, p1_model.alpha, p1_model.beta,
        #               p2_model.nu, p2_model.alpha, p2_model.beta]
        opt_params += [p1_model.nu, p1_model.alpha, p1_model.beta]
    ...
    for i in tbar:
        ...
        if i % vis_freq == 0 and i > 0:
            ...
            np.save(f'{vis_dir}/residual_{i}.npy', est_residual)
            if der_loss:
                np.save(f'{vis_dir}/nu_{i}.npy', nu.squeeze(0).detach().cpu().numpy())
                np.save(f'{vis_dir}/alpha_{i}.npy', alpha.squeeze(0).detach().cpu().numpy())
                np.save(f'{vis_dir}/beta_{i}.npy', beta.squeeze(0).detach().cpu().numpy())
