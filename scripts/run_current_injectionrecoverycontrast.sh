...
#SBATCH --mem-per-cpu=40G ## how much RAM do you need per CPU, also see --mem=<XX>G for RAM per node/computer (this effects your FairShare score so be careful to not ask for more than you need))
#SBATCH --job-name=inj2  ## When you run squeue -u NETID this is how you can identify the job
...
## lr = 0.000000004 was okay
## Attempt one at super small fov:
##CUDA_VISIBLE_DEVICES=0 python run_JWST_current_smaller_FOV_decenter.py \

CUDA_VISIBLE_DEVICES=0 python run_JWST_current_decenter_winjection.py \
        --data_dir ./HR8799stuff/before_data_newversion  \
        --measurement_file HR8799_original/target_006_00001.npy  \
        --reference_file REF-HD220657_original/reference_00001.npy \
        --scene_name HR8799_ref_only_NEWDISPERSION_injrec\
        --px_mask_file REF-HD220657_original/pixel_masks.npy \
        --ref_which_int 0 --sci_which_int 0 \
        --exp_name ref1_dith1_int0_longcurve_higherlr_noblurbefsignal \
        --iters 2500 --sci_targ_name HR8799 --ref_cutoff_iter 30000 \
        --star_offset_x 0.9 --star_offset_y 0.3 --no_median --num_radii 10 --min_radii_px 3\
        --oversample 2 --num_wl 21 --num_det_px 80 --num_ints 1 --lr 0.000000002 --stage_fluxpos_cutoff_iter 200  #--use_simulated_data  #--lr 0.00000003 #--smooth 0.6 #--use_linear_interp_opd  # --OPD_loss_weight 1 --use_ptt --use_ptt --use_linear_interp_opd
