...
def run_optimization(data_dir, root_dir, scene_name,exp_name, star_offset_x, star_offset_y, use_ptt,reference_file, measurement_file,lr,iters, vis_freq,ref_cutoff_iter, num_wl, oversample,num_det_px,sci_targ_name,num_ints,ref_which_int, sci_which_int,use_linear_interp_opd,no_median,measurement_noisemap,reference_noisemap,use_simulated_data,px_mask_file,fit_Lyot,fit_everything,stage_fluxpos_cutoff_iter,smooth,OPD_loss_weight, injected_companion=None):
    ...
    if injected_companion is not None:
        ...
        pred_planet_scaled = pred_planet_np * injected_companion['flux'] / torch.clone(peak_flux_star).detach().numpy() # CHECK THIS AGAIN
    ...
    for i in tbar:
        ...
        if i > cutoff_iter:
            ...
            if i < (stage_fluxpos_cutoff_iter + cutoff_iter):
                ...
                #     if t.requires_grad:
                #         t.grad *= 0.01
        if i < stage_fluxpos_cutoff_iter:
    ...
    np.save(f'{vis_dir}/last_iteration_ENTRANCE_OPD_oversample_{oversample}_wl_sampling_{num_wl}.npy',opd_vis_arr_numpys)
    np.save(f'{vis_dir}/last_iteration_ENTRANCE_OPD_OFFSET_oversample_{oversample}_wl_sampling_{num_wl}.npy',opd_vis_offset_arr_numpys)

    np.save(f'{vis_dir}/last_iteration_ANGLES_OFFSET_oversample_{oversample}_wl_sampling_{num_wl}.npy',angles_offset_res_numpys)
    np.save(f'{vis_dir}/last_iteration_FLUXCORRECTION_oversample_{oversample}_wl_sampling_{num_wl}.npy',progress_fluxcorrection_numpys)
    

    if num_ints > 1:
        np.save(f'{vis_dir}/last_iteration_ENTRANCE_OPD_oversample_{oversample}_wl_sampling_{num_wl}_1.npy',opd_vis_arr_numpys1)
        np.save(f'{vis_dir}/last_iteration_ANGLES_OFFSET_oversample_{oversample}_wl_sampling_{num_wl}_1.npy',angles_offset_res_numpys_1)
        np.save(f'{vis_dir}/last_iteration_ENTRANCE_OPD_OFFSET_oversample_{oversample}_wl_sampling_{num_wl}_1.npy',opd_vis_offset_arr_numpys_1)
    ...
    np.save(f'{vis_dir}/last_iteration_NIRCAM_OPD_oversample_{oversample}_wl_sampling_{num_wl}.npy',nircam_opd_vis_arr_numpys)
    if num_ints > 1:
        np.save(f'{vis_dir}/last_iteration_NIRCAM_OPD_oversample_{oversample}_wl_sampling_{num_wl}_1.npy',nircam_opd_vis_arr_1_numpys)
        

    np.save(f'{vis_dir}/first_iteration_ENTRANCE_OPD_oversample_{oversample}_wl_sampling_{num_wl}.npy',opd_vis_arr_numpys_initial)
    if num_ints > 1:
        np.save(f'{vis_dir}/first_iteration_ENTRANCE_OPD_oversample_{oversample}_wl_sampling_{num_wl}_1.npy',opd_vis_arr_numpys1_initial)
        # np.save(f'{vis_dir}/last_iteration_ENTRANCE_OPD_oversample_{args.oversample}_wl_sampling_{args.num_wl}_2.npy',opd_vis_arr_numpys2)
    np.save(f'{vis_dir}/first_iteration_NIRCAM_OPD_oversample_{oversample}_wl_sampling_{num_wl}.npy',nircam_opd_vis_arr_numpys_initial)
    ...
    np.save(f'{vis_dir}/last_iteration_oversample_{oversample}_wl_sampling_{num_wl}.npy', target_numpys[-1])
    np.save(f'{vis_dir}/last_iteration_REFERENCE_oversample_{oversample}_wl_sampling_{num_wl}.npy', reference_numpys[-1])
    
    np.save(f'{vis_dir}/last_iteration_MODEL_oversample_{oversample}_wl_sampling_{num_wl}.npy', pred_scaled.detach().cpu().numpy())
    ...
    #                 'FFMPEG', **{'macro_block_size': None, 'fps': 30, })
    if ref_cutoff_iter > iters:
        return reference_numpys[-1]
    else:
        return progress_arr_target[-1].cpu().numpy()
...
if __name__ == "__main__":
    ...
    if args.num_radii is not None:
        ...
        if args.px_mask_file is not None:
            seps, contrasts_initial = meas_contrast(masked_residual*px_mask_nanned, 0, int(args.num_det_px/2), int(int(args.num_det_px/2)/args.num_radii), low_pass_filter=True)
        else:
            seps, contrasts_initial = meas_contrast(masked_residual, 0, int(args.num_det_px/2), int(int(args.num_det_px/2)/args.num_radii), low_pass_filter=True)
        ...
        for i in range(args.num_radii):
            ...
            for j in range(args.num_angles):
                ...
                # contrasts_mask_cal_initial = contrasts_initial / contrast_new_test_weighing
                curr_flux*=1000. # TODO REMOVE LATER
                planet_injection = {'pos_x_px': pos_x_px, 'pos_y_px': -pos_y_px, 'flux': curr_flux, 'radius':i,'theta':j, 'iter_num':0}
                residual = run_optimization(args.data_dir, args.root_dir, args.scene_name,args.exp_name, args.star_offset_x, args.star_offset_y, args.use_ptt,args.reference_file, args.measurement_file,args.lr,args.iters, args.vis_freq,args.ref_cutoff_iter, args.num_wl, args.oversample,args.num_det_px,args.sci_targ_name,args.num_ints,args.ref_which_int, args.sci_which_int,args.use_linear_interp_opd,args.no_median,args.measurement_noisemap,args.reference_noisemap,args.use_simulated_data,args.px_mask_file,args.fit_Lyot,args.fit_everything,args.stage_fluxpos_cutoff_iter,args.smooth,args.OPD_loss_weight, injected_companion=planet_injection)
                
                torch.cuda.empty_cache()
                # Mask real companion
                # x_pos_real, y_pos_real = -6.5 * pixel_scale, 11 * pixel_scale # hand tuned for HIP 65426 in the pre-rotated frame
                # print('-------circular mask')
                masked_residual = residual #circular_mask(residual, x_pos_real, y_pos_real, 0.95, np.nan)
                if args.px_mask_file is not None:
                    curr_snr, annulus, signal_blob = calc_snr(masked_residual*px_mask_nanned, pos_x_px*pixel_scale, -pos_y_px*pixel_scale,0.3,curr_rad_px*pixel_scale)
                else:
                    curr_snr, annulus, signal_blob = calc_snr(masked_residual, pos_x_px*pixel_scale, -pos_y_px*pixel_scale,0.3,curr_rad_px*pixel_scale)
            ...
            if mean_snr_currad < 5. or mean_snr_currad > 6.:
                ...
                while too_faint_or_bright:
                    ...
                    for j in range(args.num_angles):
                        ...
                        # x_pos_real, y_pos_real = -6.5 * pixel_scale, 11 * pixel_scale # hand tuned for HIP 65426 in the pre-rotated frame
                        # print('-------circular mask')
                        masked_residual =  residual#circular_mask(residual, x_pos_real, y_pos_real, 0.45, np.nan)
                        if args.px_mask_file is not None:
                            curr_snr, annulus, signal_blob = calc_snr(masked_residual*px_mask_nanned, pos_x_px*pixel_scale, -pos_y_px*pixel_scale,0.3,curr_rad_px*pixel_scale)
                        else:
                            curr_snr, annulus, signal_blob = calc_snr(masked_residual, pos_x_px*pixel_scale, -pos_y_px*pixel_scale,0.3,curr_rad_px*pixel_scale)
        ...
        plt.figure()
        plt.plot(radii_list_px * pixel_scale, flux_list)
        plt.xlabel('Separation [arcsec]')
        plt.ylabel('5 sigma curve [MJy/sr]')
        plt.yscale('log')
        plt.grid()
        plt.savefig(f'{vis_dir}/detection_limit_CALIBRATED.png')
        plt.close()
