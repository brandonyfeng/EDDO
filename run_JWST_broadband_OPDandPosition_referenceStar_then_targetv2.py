...
class PointPropagate(nn.Module):
    def __init__(self, aperture, lyot, fpm, nircam_opd, args,oversample=None, use_ptt = None):
        super().__init__()
    ...
    def forward(self, wavefront_list, wfe, wl_weights, wavelenghts):
        ...
        for i in range(len(wl_weights)):
            ...
            if self.oversample != 1:
                ...
                out = torch.sum(torch.reshape(out, (1,80,self.oversample,80,self.oversample)), (2,4))
...
if __name__ == "__main__":
    ...
    if args.use_ptt:
        ...
        prop_models = [PointPropagate(aperture, lyot, fpm, nircam_OPD, prop_args,oversample=args.oversample, use_ptt=entrance_OPD_PTT).to(DEVICE) for _ in range(1)]
    else:
        prop_models = [PointPropagate(aperture, lyot, fpm, nircam_OPD, prop_args,oversample=args.oversample).to(DEVICE) for _ in range(1)]

    
    # FIRST OPTIMIZE WITH THE REFERENCE IMAGE
    real_im = np.load(f'{args.data_dir}/real_data/{args.reference_file}')[:1, 120:200, 120:200]
